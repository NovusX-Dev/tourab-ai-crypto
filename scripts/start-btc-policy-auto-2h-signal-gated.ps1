...
$serverOut = Join-Path $root "logs\btc-policy-auto-2h-signal-gated-server-$timestamp.out.log"
$serverErr = Join-Path $root "logs\btc-policy-auto-2h-signal-gated-server-$timestamp.err.log"
$runOut = Join-Path $root "logs\btc-policy-auto-2h-signal-gated-progress-$timestamp.out.log"
$runErr = Join-Path $root "logs\btc-policy-auto-2h-signal-gated-progress-$timestamp.err.log"
...
$run = Start-Process -FilePath "npm.cmd" `
  -ArgumentList "exec", "--", "tsx", "scripts/policy-auto-progress-run.ts", "--label", "btc-policy-auto-2h-signal-gated", "--base-url", "http://localhost:7071", "--duration-sec", "7200", "--drain-sec", "900", "--poll-ms", "5000", "--progress-sec", "60" `
  -WorkingDirectory $root `
